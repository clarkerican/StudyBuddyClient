...
public class Collaborator implements TextWatcher {
    ...
    private int cursorPosition, cursorStart;
    ...
    public void process(Mutation mutation){
        ...
        if (mutation.senderID.equals(myID)) {
            ...
            try {
                ...
                cursorStart = textField.getSelectionStart();
                Log.d(Mutation.TAG, "Before: \"" + textField.getText().toString().substring(0, cursorStart)
                        + "|" + textField.getText().toString().substring(cursorStart) + "\"");
                ...
                switch (mutation.type) {
                    case Mutation.MUTATION_INSERT:
                        ...
                        sBuilder.insert(mutation.index, ((Insert) mutation).toInsert);
                        text = sBuilder.toString();
                        // Change cursor position
                        if (mutation.index <= cursorStart) {
                            cursorStart += ((Insert) mutation).toInsert.length();
                        }
                        break;
                    case Mutation.MUTATION_DELETE:
                        Log.d(Mutation.TAG, "Deleting " + ((Delete) mutation).numChars + " characters at index " + mutation.index);
                        sBuilder.delete(mutation.index, mutation.index + ((Delete) mutation).numChars);
                        text = sBuilder.toString();
                        // Change cursor position
                        if (mutation.index < cursorStart) {
                            if (mutation.end() < cursorStart) {
                                cursorStart -= ((Delete) mutation).numChars;
                            }
                            else {
                                cursorStart = mutation.index;
                            }
                        }
                        break;
                    default:
                ...
                Log.d(Mutation.TAG, "After:  \"" + textField.getText().toString().substring(0, cursorStart)
                        + "|" + textField.getText().toString().substring(cursorStart) + "\"");
                textField.setSelection(cursorStart);
            }
